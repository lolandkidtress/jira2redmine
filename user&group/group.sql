...
exclude jira-users,jira-developers,jira-administrators
*/
insert into  bitnami_redmine.groups_users
(group_id,user_id)
select groupid,userid from 
(select 
	(select redmine.id  from bitnami_redmine.users redmine
		where redmine.type ='Group'
		and redmine.lastname = parent_name
		)as groupid,
	(select distinct(redmine.id) from bitnami_redmine.users redmine
